...
ggplot() +
  ...
  coord_sf(expand = FALSE, xlim = c(-20, 150), ylim = c(10, 75)) +
  ...
  labs(
    title = "Ancient DNA Discovery Sites and Latitude"
  )
...
```{r modern_samples, echo = FALSE, message = FALSE, warning = FALSE, include = FALSE}
metadata <- read.table(
...
```{r sgdp20_map, echo = FALSE, message = FALSE, warning = FALSE}
...
modern_sf <- st_as_sf(modern_samples, coords = c("lon","lat"), crs = 4326)
...
To compare genome-wide ancestry patterns against the structure driven by a targeted functional SNP set, pigmentation-specific analyses used a panel of 222 SNPs previously associated with pigmentation phenotypes in the NHGRI-EBI Catalog of human genome-wide association studies. The by-chromosome distribution of these SNPs is shown below.
...
snp_tab <- data.frame(
  Locus = c("SLC24A5", "SLC45A2", "TYR", "OCA2/HERC2", "MC1R and ASIP"),
...
```{r vocab_table, echo = FALSE, message = FALSE, warning = FALSE}
vocab_tab <- data.frame(
  Term = c("Loci", "Allele", "Haplotype", "Genotype"),
  Definition = c("Position of a gene or DNA sequence on a chromosome", "One form of a gene", "A set of closely linked genetic markers on a single chromosome that are inherited together from a single parent", "Two alleles present in an individual at a specific locus of the genome")
)
knitr::kable(vocab_tab, caption = "Vocabulary")
```
