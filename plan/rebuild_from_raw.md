...
**Finalized sample subset (2026-07-23) — full justification in [`papers/SAMPLE_INCLUSION.md`](papers/SAMPLE_INCLUSION.md).** The goal is to maximize *reliably-genotyped pigmentation SNPs of interest*, not genome count. **Tier 1 (core):** the 5 high-coverage genomes (Altai/Denisova 5, Vindija 33.19, Denisova 3, Chagyrskaya 8, Denisova 25) + SGDP — they cover all 222 panel SNPs incl. the transition loci reliably. **Tier 2 (extended, with GL uncertainty):** Les Cottés, Denisova 11 (hybrid), Goyet, and Mezmaiskaya 1 (its snpAD VCF). **Excluded, with reasons:** El Sidrón (exome-only), Vindija 87 (redundant = Vi 33.19 individual), Mezmaiskaya 2 (1.7×, no VCF), Spy 94a (1.0× + ~4% contamination), Hohlenstein-Stadel & Scladina (ultra-low ~0.02–0.05× → only ~4–11 panel SNPs, contamination-dominated). So **targeted read re-acquisition is needed for just 3 genomes** (Les Cottés, Denisova 11, Goyet).
...
- **Re-acquire reads for just 3 genomes** (Les Cottés, Denisova 11, Goyet) — **targeted** extraction at the SNP positions; the 5 high-coverage genomes need **no re-acquisition and no re-calling** (download the published VCFs), Mezmaiskaya 1 uses its VCF, and 6 genomes are excluded (see `SAMPLE_INCLUSION.md`).
