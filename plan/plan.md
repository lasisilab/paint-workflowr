...
- **The genome-build mismatch (see item A2) — RESOLVED: hg19.** The pigmentation SNP list is on hg38 while the sequence data is on hg19. **Decided (2026-07-23): standardize on hg19** — the data is hg19-native and the archaic genomes are published only as hg19 BAMs, so we lift the panel (and the four hg38 SNP-lists) *down* to hg19 rather than re-aligning terabytes of reads up to hg38. The genome-wide branch is already hg19-verified; the panel lift is the remaining piece. (Override only if hg38 becomes the committed standard — see B2b.)
...
The **single most unblocking step** is **B2** (put everything on **hg19** — the build all the sequence data already uses): it is literally the fix for the root bug **A2**. The four new pigmentation datasets (hg38) are small SNP-lists lifted **down** to hg19 for integration (a full migration up to hg38 is deferred and optional — see B2). **A1** (Denisova 3) is an independent naming fix that can happen in parallel.
...
- [ ] **A2 · The SNP list and the sequence data are on different genome builds (hg38 vs hg19) — the root problem** · 🔴 (TL/LH) · **CONFIRMED · TODO (fixed by B2)**
  ...
  - **Do:** put the panel and the sequence data on the *same* build before re-running any pigmentation-SNP analysis. The chosen direction (item B2) is to lift the **panel down to hg19** — the sequence data is already on hg19, and the archaic genomes are published only as hg19-aligned BAMs, so keeping the data native and moving the 222-SNP panel is the minimal, lossless fix. Lift by **rsID** (re-pull each SNP's hg19 coordinate from dbSNP, keeping both builds so this can't recur) or with `liftOver`/`CrossMap` + an hg38→hg19 chain file; the four hg38 pigmentation SNP-lists lift down the same way. Until the panel is on hg19, treat the pigmentation PCA and pigmentation-SNP depth results as unreliable.
...
- [ ] **B2 · Put everything on one genome build: hg19** · 🔴 (TL/LH) · **TODO (critical path)**
  - **What it is:** as item A2 shows, the SNP panel is on hg38 but the sequence data is on hg19, which is why the pigmentation analyses are broken. Standardize the whole project on **hg19** — the build all the sequence data already uses. Because the archaic genomes are published only as hg19-aligned BAMs (re-aligning to hg38 would need the raw FASTQ + a full ancient-DNA re-alignment, which is heavy and partly unavailable), the minimal correct fix is to lift the 222-SNP panel — and the four hg38 pigmentation SNP-lists (B1) — **down** to hg19 to match the data.
  - **Why it matters:** this is the fix for the root bug (A2) *and* the prerequisite for every matched-build analysis (B1, B3, B4, B7, Q9); the four hg38 datasets are lifted down to hg19 for integration. Nothing pigmentation-SNP-related is trustworthy until builds match.
  - **Do:** lift the panel (and the four hg38 SNP-lists) down to hg19 by rsID; keep the sequence data native on hg19. Then re-run depth + PCA. **This is the critical-path first step.** (The genome-wide branch is *already* hg19-consistent — verified by the tracer SNP rs1426654 at `15:48426484` in the SGDP + archaic data — so this build fix now applies specifically to the pigmentation-panel arm.)
  ...
  - **Execution plan — lift the panel to hg19 (the data's native build):**
    - **B2a · Panel → hg19 (the chosen route — fast, lossless).** The data is *already* on hg19, so the minimal correct fix is to move the 222-SNP panel onto hg19. Steps: (1) convert the panel's coordinates hg38→hg19 — either `liftOver`/`CrossMap` on `snps.bed` with the UCSC chain file, or (recommended, more robust) re-pull each SNP's hg19 coordinate by **rsID** from dbSNP and store **both** builds so this bug can never recur; (2) verify with **Q2** — the tracer SNP rs1426654 must now land on `15:48426484` (hg19), and the naming lint must pass; (3) re-run `samtools depth` over the hg19 panel → new `*_depth.txt`; (4) re-extract SGDP + archaic genotypes at the hg19 panel and rebuild the pigmentation PCA; (5) re-run the projection **with `no-mean-imputation`** (the Q5 fix in `pca_proj.py`); (6) apply **A1** in the same pass (reheader Denisova 3 to bare names, re-run its depth + calls); (7) re-run `plan/figures/make_consequence_figures.R after` to produce the **after** figures. Outcome: a correct pigmentation coverage + PCA + a real before/after comparison, at low compute — this validates the entire pigmentation arm on the build the data already uses. **Full step-by-step commands, the SLURM script, the compute ask, and exactly what's needed from the team: [`b2a_runbook.md`](b2a_runbook.md).**
    - **B2b · Data → hg38 (optional, deferred — NOT the current plan).** Only if hg38 later becomes the committed project standard. It is *not* needed for integration: the four hg38 pigmentation datasets are small SNP-lists lifted *down* to hg19 (B2a), so no bulk data migration is required. Documented only as the fallback — it would mean re-aligning the public raw reads to GRCh38 (heavy, and partly blocked because MPI-EVA publishes only processed hg19 BAMs), then re-verifying with Q2/Q4/Q3/Q5/Q6.
    - **Recommended order:** **B2a first** (a one-to-two-day integrity check that produces the before/after the PI asked for), **then B2b** for the science phase. If the team prefers a single migration, jump straight to B2b — but B2a is the fastest way to *prove* the fix works and get the comparison figures. *(This is the one open choice in B2; default = B2a-then-B2b.)*
...
Plus the call's "normalize everything to one build" → **B2** (Phase 1; the fix for bug A2) — **now resolved as hg19** (the data's native build; see §3 and B2), not hg38.
