...
- **Contig:** short for "contiguous sequence" — one continuous stretch of the reference genome that reads are aligned against. In a human reference, each chromosome is (essentially) one contig, plus extra small "unplaced" and "decoy" contigs. Every BAM and VCF begins with a header line **per contig**: `@SQ SN:<name> LN:<length>` (e.g. `@SQ SN:1 LN:249250621`). The `SN` value is the contig's **name/label**. **"Contig naming"** is whether those labels are written bare (`1`, `2`, … `X`) or with a prefix (`chr1`, `chr2`, … `chrX`). Two files can describe the *same* genome yet be unusable together if one calls chromosome 1 `1` and the other `chr1` — the software matches by name, finds no overlap, and silently returns nothing. That mismatch on one sample is exactly bug **A1** (Denisova 3's BAM was `chr`-named while the reference and panel were bare-named).
- **Genotype likelihood (GL) vs hard call:** a **hard call** commits to one definite genotype per position (e.g. "this individual is `A/G`"). A **genotype likelihood** instead keeps the *probability* of each possible genotype given the reads — so a position covered by a single read isn't forced into a false certainty; the uncertainty is carried forward. For **low-coverage / damaged ancient DNA**, hard-calling throws away or fabricates most genotypes, whereas GLs let even a 1-read site contribute partial information. GL-based methods are the appropriate choice for archaic samples (see ANGSD).
- **ANGSD / PCAngsd:** **ANGSD** (Analysis of Next Generation Sequencing Data) computes **genotype likelihoods** directly from BAM files without hard-calling, with built-in aDNA options (base/mapping-quality filters, read-end trimming, and `-noTrans` to use transversions only). **PCAngsd** builds a PCA from those genotype likelihoods, propagating per-sample uncertainty — the standard way to do PCA on low-coverage data. This is the primary analysis engine for the rebuild (see [`rebuild_from_raw.md`](rebuild_from_raw.md)); `bcftools` hard-calling is reserved for the high-coverage samples and specific per-SNP genotype reporting.
...
**Bug evidence & before/after figures:** every bug is reproduced *and* traced to its downstream consequence in [`verify/BUG_EVIDENCE.md`](verify/BUG_EVIDENCE.md). The current (buggy-state) figures live in [`plan/figures/`](figures/) as `before_*.png` — a rank-1 scree, the collapsed pigmentation projection, and Denisova 3's artifactual coverage — and are re-generated as `after_*.png` once the fix lands, by re-running [`figures/make_consequence_figures.R`](figures/make_consequence_figures.R). Keeping the before/after side-by-side is how we demonstrate (not just assert) that fixing the build changed the results.

**Data & pipeline (living record):** what the raw data is, the BAM-vs-VCF distinction, what each pipeline step transforms, and a run log updated as we execute — [`pipeline.html`](pipeline.html) (shareable; linked from the roadmap).

**Rebuild decision (2026-07-23):** because the derived data has confirmed QC failures (A1/A2/A3, the projection collapse) *and* the full sorted BAMs are no longer on the cluster, we do **not** trust the existing derived products — we **rebuild the pipeline from the raw, re-downloaded public BAMs** with a QC gate at every stage. The staged, compute-estimated plan is [`rebuild_from_raw.md`](rebuild_from_raw.md); it executes Phases 0–4 below *from raw* and supersedes the "patch the panel" quick route in [`b2a_runbook.md`](b2a_runbook.md) (kept as an optional 1-hour preview). Cluster access is the only dependency; no data is needed from the team. **Reuse-vs-rerun logic (the specifics):** the 5 high-coverage genomes need **no re-acquisition and no re-calling** — we take the published damage-aware `snpAD` genotype VCFs and only harmonize/lint them (applying each release's companion FilterBed, then intersecting to a common site set). **Decided subset (2026-07-23; goal = maximize reliably-genotyped pigmentation SNPs, not genome count — full justification in [`papers/SAMPLE_INCLUSION.md`](papers/SAMPLE_INCLUSION.md)):** **Tier 1 (core)** = the 5 high-coverage genomes + SGDP (cover all 222 panel SNPs incl. the transition loci reliably); **Tier 2 (extended, GL uncertainty)** = Les Cottés, Denisova 11, Goyet + Mez1 (its VCF); **excluded** = El Sidrón (exome), Vindija 87 (redundant), Mez2 (1.7×), Spy (1.0× + contamination), Hohlenstein-Stadel & Scladina (ultra-low). So **targeted re-acquisition is needed for only 3 genomes**; **SGDP is verified (not rebuilt)**. The per-genome matrix is in [`rebuild_from_raw.md`](rebuild_from_raw.md) ("Reuse vs. rerun"); what each published call set's QC actually was is in [`papers/PIPELINE_QC_BY_PAPER.md`](papers/PIPELINE_QC_BY_PAPER.md).

## 3. Open questions for Lily (please confirm)
- **The genome-build mismatch (see item A2) — RESOLVED.** The pigmentation panel + the gene-network datasets are hg38; the archaic/SGDP genotypes are physically hg19. **Decided (2026-07-23): report and integrate on hg38** (the field standard + where the gene-network lives), bridging to the hg19 data by a verified rsID↔hg19↔hg38 map — extract genotypes at the hg19 coordinate, express on hg38 (a genotype is build-independent, so this is lossless relabeling, not re-alignment). No genome-wide re-alignment is needed because the analysis is locus-focused. The genome-wide sanity PCA stays hg19 as a control.
...
The **single most unblocking step** is **B2** (establish **hg38** as the reporting build via a verified rsID↔hg19↔hg38 locus map that bridges the hg19-native genotypes): it is the fix for the root bug **A2** *and* the bridge that lets the four hg38 gene-network datasets (**B1**) integrate on a common coordinate system. It relabels coordinates rather than re-aligning reads, so it's cheap. **A1** (Denisova 3) is an independent naming fix that can happen in parallel.
...
- [ ] **A2 · The SNP list and the sequence data are on different genome builds (hg38 vs hg19) — the root problem** · 🔴 (TL/LH) · **CONFIRMED · TODO (fixed by B2)**
  ...
  - **Do:** stop reading hg38 panel positions out of hg19 data. Build a **verified rsID ↔ hg19 ↔ hg38 map** for the panel (item B2): re-pull each SNP's coordinate in *both* builds from dbSNP (keyed by rsID) with per-build ref/alt, extract the genotype from the hg19-aligned data at the **hg19** coordinate, and report/integrate it on **hg38** (the build the project + the gene-network use). This relabels coordinates, it does not re-align reads — lossless for any SNP with an rsID. Until that map is in place and Q2 passes, treat the pigmentation PCA and pigmentation-SNP depth results as unreliable.
...
- [ ] **B2 · One reporting build (hg38), bridged to the hg19 data by a verified locus map** · 🔴 (TL/LH) · **TODO (critical path)**
  - **What it is:** the SNP panel — and the pigmentation-gene-network datasets (B1) — are on **hg38**; the archaic + SGDP genotypes are physically on **hg19**. The project **reports and integrates on hg38** (the field standard, and where the gene-network + GWAS Catalog live), but we do *not* re-align the genotype data. Because the analysis is **locus-focused** (a defined set of pigmentation SNPs/genes, not genome-wide), we build a **verified rsID ↔ hg19 ↔ hg38 map** — both coordinates + per-build ref/alt for every panel and gene-network SNP — read genotypes from the hg19 data at the **hg19** coordinate, and express everything on **hg38**.
  ...
  - **Do:** build the rsID dual-coordinate locus map (panel + gene-network SNPs) with per-build ref/alt; extract SGDP + archaic genotypes from the hg19 data at each locus; report + re-run depth + PCA on **hg38**. **This is the critical-path first step.** (The genome-wide *sanity* PCA is a control and stays on hg19 — its coordinates never integrate externally, so it needs no mapping; verified by the tracer SNP rs1426654 at `15:48426484`.)
  - **Evidence:** its correctness is *proven* by Q2 (tracer-SNP lint passes) and Q5 (PCA no longer collapses) — B2 isn't "done" until both pass. The **before** state is captured in `plan/figures/before_*.png` (rank-1 scree, collapsed projection, Denisova-3 coverage); the fix's success is the matching `after_*.png`.
  - **Do we need any new data? No.** Everything required is already in place or is public and fetchable — nothing is needed from Lily, Yemko, or any collaborator. Specifically: the archaic + SGDP BAMs (hg19) already sit on the cluster (110 GB); the 222-SNP panel is committed (`data/snps.txt`) and its rsIDs are in `data/skin_pigmentation.tsv`; the UCSC `hg38ToHg19.over.chain.gz` chain file and (for the hg38 route) the GRCh38 reference + the public raw reads (MPI-EVA / EBI-ENA for the archaics, SGDP for the moderns) are all public downloads. **The only actual dependency is cluster compute access** — the authenticated Great Lakes SSH session (UMich Kerberos + Duo), which just needs re-establishing (it drops when the connection times out). No missing dataset blocks this.
  - **Execution plan — build the verified hg19↔hg38 locus map, report on hg38:**
    - **B2a · Pigmentation loci on hg38 via the verified map (the route — fast, lossless).** Steps: (1) for each panel SNP (and, for B1, each gene-network SNP), re-pull **both** the hg38 and hg19 coordinates + ref/alt **by rsID** from dbSNP, storing both builds so the mismatch can never recur; (2) verify with **Q2** — the tracer rs1426654 must sit at `15:48426484` (hg19) and `15:48134288` (hg38), naming consistent; (3) re-run `samtools depth` at the **hg19** coordinates → new `*_depth.txt`; (4) extract SGDP + archaic genotypes from the hg19 data at the hg19 coordinates and rebuild the pigmentation PCA, **labelled on hg38**; (5) re-run the projection **with `no-mean-imputation`** (the Q5 fix in `pca_proj.py`); (6) apply **A1** in the same pass (reheader Denisova 3, re-run its depth + calls); (7) re-run `plan/figures/make_consequence_figures.R after` for the **after** figures. Outcome: the pigmentation arm correct and reported on hg38, ready to integrate the B1 gene-network data — plus a real before/after, at low compute. **Commands, SLURM, compute ask: [`b2a_runbook.md`](b2a_runbook.md).** *(A genotype is build-independent — this relabels coordinates by rsID, it does not recompute genotypes, so it is lossless for any SNP with an rsID.)*
    - **B2b · Genome-wide genotypes physically on hg38 (optional, deferred — only if ever needed).** The locus-focused deliverable does **not** need this: B2a already reports on hg38 at every panel + gene-network SNP without touching genome-wide data. Re-aligning the archaic reads to GRCh38 is the *only* thing that's hard (needs raw FASTQ; MPI-EVA publishes only hg19 BAMs) — and it is only required to *discover novel variants* genome-wide on hg38, which is not a project goal. If a genome-wide hg38 product ever becomes necessary, the lighter route is to lift the *called* VCFs hg19→hg38 with `CrossMap`/GATK `LiftoverVcf` (fast, ~1–2% site loss). The genome-wide sanity PCA stays hg19 as a control regardless.
    - **Recommended order:** B2a is the route — hg38 reporting + before/after at low compute, and it unblocks B1 integration. B2b is invoked only if a genome-wide hg38 product later proves necessary. *(Open choice: whether B2b is ever needed; default = B2a only.)*
...
Plus the call's "normalize everything to one build" → **B2** (Phase 1; the fix for bug A2) — **resolved as: report/integrate on hg38**, bridged to the hg19-native genotypes by a verified rsID dual-coordinate map (see §3 and B2).
...
- [ ] **B4 · Redo the PCAs three ways, two designs each** · 🟡 (LH) · **BLOCKED(B2)**
  ...
  - **Method note (low coverage → genotype likelihoods):** the archaics are mostly low-coverage (10 of 14 under ~3.2× at the panel — exact table in [`coverage_current.tsv`](coverage_current.tsv)), so the PCA is built with **genotype likelihoods via ANGSD/PCAngsd**, not hard-called genotypes — this is the low-coverage-appropriate method and replaces the `plink2 --pca`/`--score` hard-genotype path that produced the collapse. Hard calls are used only for the high-coverage genomes. See the rebuild pipeline ([`rebuild_from_raw.md`](rebuild_from_raw.md) Stage 4/6).
